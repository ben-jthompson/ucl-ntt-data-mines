...
class Querier:
    ...
    - If you cannot find relevant data from the `query_vector_db' tool after reformulating queries three times, call the `web_search` tool to gather external context from the web. 
    - Only if both `query_vector_db` and `web_search` fail to provide relevant results, rely on your own knowledge to provide an informed answer. 
    ...
    - Before producing the Final Answer, you MUST call the `add_source_to_bibliography` tool once for each source you cite. The tool takes a single string input: the redirect URL of the source (or, if from Tavily search, the URL of the result).
